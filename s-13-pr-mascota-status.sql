...
begin
 ...
 if v_count != 0 then 
  select tipo_id into v_id where nombre = v_nombre and subcategoria = v_subcategoria 
...
begin
 ...
 v_tipo_id := exec pr_obtieneTipoid(v_tipo, v_subcategoria, v_nivel_cuidados);
 ...
 if v_cliente_username is not null then
  select cliente_id into v_cliente_id from cliente where username = v_cliente_username;
   insert into mascota 
    (mascota_id, nombre, folio, fecha_status, fecha_ingreso, fecha_nacimiento, origen,tipo_id,status_id,centro_operativo_id)
  ...
  if v_padre_folio is not null then
   ...
   insert into nacito values (v_id, v_madre_id,v_padre_id,v_nacido_id);
  else 
