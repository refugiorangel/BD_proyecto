...
before insert or update of ganador, cliente_id,mascota_id on mascota_cliente
for each row
declare 
 v_count number;
 v_id number;
begin
 v_id := :new.cliente_id;
 select coun(*) into v_count from mascota_cliente where cliente_id =v_id and ganador = 1;
 ...
 v_id := :new.mascota_id;
 select count(*) into v_count from mascota_cliente where mascota_id = v_id and ganador = 1;
